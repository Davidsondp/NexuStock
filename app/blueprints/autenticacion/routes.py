...
def _seleccion_registro():
    if request.method == "GET" and ("plan" in request.args or "ciclo" in request.args):
        ...
        if not plan or ciclo not in CICLOS_COMERCIALES:
    ...
    if not codigo or ciclo not in CICLOS_COMERCIALES:
        _limpiar_seleccion_registro()
        return None, None
...
def registro():
    ...
    plan_seleccionado, ciclo_seleccionado = _seleccion_registro()
    ...
    if formulario.validate_on_submit():
        try:
            ...
            login_user(usuario)
            flash(
                "Tu empresa fue creada " "correctamente.",
                "exito",
            )
...
def reenviar_verificacion():
    ...
    if not current_user.email_verificado:
        emitir_verificacion(current_user)
        flash("Enviamos un nuevo enlace de verificación.", "exito")
