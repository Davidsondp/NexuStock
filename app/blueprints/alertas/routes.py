...
def listar():
    ...
    alertas = servicio.listar(estado=request.args.get("estado", "activa"),
...
def generar():
    ...
    return jsonify({"creadas": resultado.creadas, "actualizadas": resultado.actualizadas,
...
def resolver(alerta_id):
    try:
        return jsonify(_serializar(ServicioAlertas(current_user).cambiar_estado(alerta_id, "resuelta")))
...
def ignorar(alerta_id):
    try:
        return jsonify(_serializar(ServicioAlertas(current_user).cambiar_estado(alerta_id, "ignorada")))
    except ErrorAlerta as exc:
        return jsonify({"codigo": exc.codigo, "mensaje": str(exc)}), 400
