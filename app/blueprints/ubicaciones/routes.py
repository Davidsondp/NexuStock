...
def _error(exc, estado=400): return jsonify({"codigo": "ubicacion_invalida", "mensaje": str(exc)}), estado
def _sucursal(s): return {"id": s.id, "codigo": s.codigo, "nombre": s.nombre, "activa": s.activa}
def _bodega(b): return {"id": b.id, "sucursal_id": b.sucursal_id, "codigo": b.codigo,
                        "nombre": b.nombre, "activa": b.activa}
...
def listar_sucursales():
    return jsonify({"sucursales": [_sucursal(s) for s in ServicioUbicaciones(current_user).listar_sucursales()]})
...
def crear_sucursal():
    try:
        ...
        permitidos = {campo: recibidos[campo] for campo in (
            "codigo", "nombre", "direccion", "ciudad", "telefono", "crear_bodega_principal"
    ...
    except ErrorUbicacion as exc: return _error(exc)
...
def desactivar_sucursal(sucursal_id):
    try:
        ServicioUbicaciones(current_user).desactivar_sucursal(sucursal_id); return "", 204
    except ErrorUbicacion as exc: return _error(exc, 409)
...
def listar_bodegas():
    ...
    return jsonify({"bodegas": [_bodega(b) for b in ServicioUbicaciones(current_user).listar_bodegas(sucursal_id)]})
...
def desactivar_bodega(bodega_id):
    try:
        ServicioUbicaciones(current_user).desactivar_bodega(bodega_id); return "", 204
    except ErrorUbicacion as exc: return _error(exc, 409)
...
def asignar_usuario(sucursal_id, usuario_id):
    ...
    return jsonify({"id": asignacion.id, "usuario_id": asignacion.usuario_id,
                    "sucursal_id": asignacion.sucursal_id,
                    "es_principal": asignacion.es_principal}), 201
