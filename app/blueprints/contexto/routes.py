...
from ...services.contexto import (bodegas_autorizadas, establecer_contexto,
...
def seleccionar():
    ...
    sucursal_solicitada = formulario.sucursal_id.data if formulario.sucursal_id.data else (
        sucursales[0].id if sucursales else 0
    )
    ...
    if formulario.validate_on_submit():
        try:
            contexto = establecer_contexto(current_user, formulario.sucursal_id.data,
                                            formulario.bodega_id.data)
            registrar_auditoria(
                accion="contexto.seleccionado", modulo="contexto",
                usuario_id=current_user.id, empresa_id=current_user.empresa_id,
                entidad_tipo="Bodega", entidad_id=contexto.bodega.id,
...
def bodegas(sucursal_id: int):
    return {"bodegas": [{"id": b.id, "nombre": b.nombre}
                         for b in bodegas_autorizadas(current_user, sucursal_id)]}
