...
class ErrorUnidadMedida(ValueError):
    """La unidad o presentaci?n no es v?lida."""
...
class ServicioUnidadesMedida:
    ...
    def listar(
        ...
        *,
        ...
        producto = self._obtener_producto(
            producto_id
        )
        ...
        if not incluir_inactivas:
            consulta = consulta.where(
                PresentacionProducto.activa.is_(
                    True
                )
            )
    ...
    def crear(
        ...
        *,
        ...
        if (
            ...
            or not abreviatura_normalizada
        ):
            raise ErrorUnidadMedida(
                "C?digo, nombre y abreviatura "
                "son obligatorios"
            )
        ...
        if codigo_normalizado in {
            ...
            raise ErrorUnidadMedida(
                "BASE y CAJA son c?digos "
                "administrados por el producto"
            )

        factor = self._decimal_positivo(
            factor_base,
            "El factor de conversi?n",
        )
        ...
        try:
            ...
            db.session.rollback()
            raise ErrorUnidadMedida(
                "La presentaci?n ya existe "
                "o contiene datos inv?lidos"
            ) from exc
        except Exception:
            db.session.rollback()
            raise
    ...
    def editar(
        ...
        *,
        ...
        self._exigir("productos.editar")
        presentacion = (
            self._obtener_presentacion(
                producto_id=producto_id,
                presentacion_id=presentacion_id,
            )
        )
        self._validar_presentacion_administrada(
            presentacion
        )
