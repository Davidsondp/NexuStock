...
class ServicioClientes:
    ...
    def listar(
        ...
        *,
        ...
        if busqueda and busqueda.strip():
            ...
            consulta = consulta.where(
                db.or_(
                    ...
                    Cliente.identificacion_fiscal.ilike(
                        patron
                    ),
        ...
        return list(
            db.session.scalars(
                consulta.order_by(Cliente.nombre)
            )
        )
    ...
    def obtener(self, cliente_id: int) -> Cliente:
        ...
        if not cliente:
            raise PermissionError(
                "Cliente no encontrado en la empresa"
            )
    ...
    def _asignar(self, cliente, datos):
        nombre = (
            datos.get("nombre", cliente.nombre)
            or ""
        ).strip()
