...
class ServicioAlertas:
    ...
    def listar(self, *, estado="activa", tipo=None, bodega_id=None):
        ...
        return list(db.session.scalars(consulta.order_by(
    ...
    def cambiar_estado(self, alerta_id: int, estado: str) -> AlertaInventario:
        ...
        alerta.estado = estado; alerta.resuelta_por_id = self.usuario.id; alerta.resuelta_en = utcnow()
        registrar_auditoria(
            accion=f"alerta.{estado}", modulo="alertas", usuario_id=self.usuario.id,
            empresa_id=self.usuario.empresa_id, entidad_tipo="AlertaInventario",
            entidad_id=alerta.id, datos_nuevos={"estado": estado},
        )
        db.session.commit(); return alerta
    ...
    def _evaluar(self, inventario, producto, ahora, dias_sin_movimiento, configuracion=None):
        ...
        minimo = Decimal(producto.stock_minimo); reorden = Decimal(producto.punto_reorden)
        maximo = Decimal(producto.stock_maximo) if producto.stock_maximo is not None else None
        base = {"stock": str(inventario.cantidad), "reservado": str(inventario.cantidad_reservada),
        ...
        if stock_bajo_habilitado and disponible <= minimo:
            ...
            reglas.append(("stock_bajo", prioridad, f"Stock bajo: {producto.nombre}",
        ...
        if sobrestock_habilitado and maximo is not None and Decimal(inventario.cantidad) > maximo:
            ...
            reglas.append(("sobrestock", "media", f"Sobrestock: {producto.nombre}",
        ...
        consumo = abs(Decimal(db.session.scalar(db.select(db.func.coalesce(db.func.sum(Movimiento.cantidad), 0)).where(
            Movimiento.empresa_id == self.usuario.empresa_id,
            Movimiento.producto_id == producto.id, Movimiento.bodega_id == inventario.bodega_id,
            Movimiento.tipo == "salida", Movimiento.fecha >= inicio, Movimiento.fecha <= ahora)) or 0))
        promedio = consumo / Decimal(self.VENTANA_CONSUMO_DIAS)
        proveedor = db.session.get(Proveedor, producto.proveedor_principal_id) if producto.proveedor_principal_id else None
        ...
        if promedio > 0:
            ...
            if cobertura <= Decimal(dias_reposicion):
                reglas.append(("riesgo_agotamiento", "alta", f"Riesgo de agotamiento: {producto.nombre}",
                               f"La cobertura estimada es de {cobertura.quantize(Decimal('0.1'))} días.",
                               {**base, "ventana_dias": self.VENTANA_CONSUMO_DIAS,
                                "consumo": str(consumo), "promedio_diario": str(promedio),
                                "dias_cobertura": str(cobertura), "dias_reposicion": dias_reposicion}))
        if disponible <= reorden:
            objetivo = maximo if maximo is not None else reorden + (promedio * Decimal(dias_reposicion))
            sugerida = max(Decimal(0), objetivo - disponible).quantize(Decimal("1"), rounding=ROUND_CEILING)
            reglas.append(("recomendacion_compra", "alta" if disponible <= minimo else "media",
                           f"Recomendación de compra: {producto.nombre}",
                           f"Se recomienda comprar {sugerida} unidades para recuperar el nivel objetivo.",
                           {**base, "punto_reorden": str(reorden), "cantidad_sugerida": str(sugerida),
                            "metodo": "nivel_objetivo", "dias_reposicion": dias_reposicion}))
        ...
        if Decimal(inventario.cantidad) > 0 and referencia <= limite:
            reglas.append(("sin_movimiento", "baja", f"Producto sin movimiento: {producto.nombre}",
    ...
    def _bodegas_autorizadas(self):
        ...
        return set(db.session.scalars(db.select(Bodega.id).where(
            Bodega.empresa_id == self.usuario.empresa_id,
            Bodega.sucursal_id.in_(sucursales), Bodega.activa.is_(True),
