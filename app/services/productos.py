...
class ServicioProductos:
    ...
    def listar(self, *, busqueda=None, incluir_inactivos=False):
        ...
        if busqueda:
            ...
            consulta = consulta.where(db.or_(Producto.nombre.ilike(patron),
    ...
    def editar(self, producto_id: int, **datos) -> Producto:
    ...
    def eliminar_logicamente(self, producto_id: int) -> Producto:
        self._exigir("productos.eliminar")
        producto = self.obtener(producto_id)
        tiene_historial = db.session.scalar(db.select(db.exists().where(
            Movimiento.empresa_id == self.usuario.empresa_id,
            Movimiento.producto_id == producto.id)))
        tiene_saldo = db.session.scalar(db.select(db.exists().where(
        ...
        producto.soft_delete(); producto.activo = False
    ...
    def _asignar(self, producto, datos):
        ...
        try:
            numeros = {campo: Decimal(str(datos.get(campo, getattr(producto, campo, None) or valor))) for campo, valor in {
                "unidades_por_caja": 1, "costo_referencia": 0, "precio_venta": 0,
                "tasa_impuesto": "0.19", "stock_minimo": 0, "punto_reorden": 0,
            }.items()}
            if "stock_maximo" in datos:
                stock_maximo = (Decimal(str(datos["stock_maximo"]))
                                if datos.get("stock_maximo") not in (None, "") else None)
            else:
        ...
        if proveedor_id:
            proveedor = db.session.scalar(db.select(Proveedor).where(
                Proveedor.id == int(proveedor_id), Proveedor.empresa_id == self.usuario.empresa_id,
    ...
    def _validar_limite(self):
        ...
        cantidad = db.session.scalar(db.select(db.func.count(Producto.id)).where(
            Producto.empresa_id == self.usuario.empresa_id,
            Producto.eliminado.is_(False)))
        if cantidad >= limite:
    ...
    def _auditar(self, producto, accion, anteriores=None):
        registrar_auditoria(
            accion=f"producto.{accion}", modulo="productos", usuario_id=self.usuario.id,
            empresa_id=self.usuario.empresa_id, entidad_tipo="Producto", entidad_id=producto.id,
            datos_anteriores=anteriores,
            datos_nuevos={"codigo": producto.codigo, "nombre": producto.nombre,
