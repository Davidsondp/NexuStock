...
def solicitar_restablecimiento(email: str) -> None:
    ...
    registrar_auditoria(
        accion="password.restablecimiento_solicitado", modulo="autenticacion",
    ...
    mensaje = Message(
        ...
        body=("Solicitaste cambiar tu contraseña de NexuStock.\n\n"
...
def restablecer_password(usuario: Usuario, token: str, password: str) -> bool:
    ...
    registrar_auditoria(
        accion="password.restablecido", modulo="autenticacion",
        usuario_id=usuario.id, empresa_id=usuario.empresa_id,
        entidad_tipo="Usuario", entidad_id=usuario.id,
    )
    db.session.commit()
