...
class ServicioProveedores:
    ...
    def crear(self, **datos) -> Proveedor:
        ...
        try:
            ...
            db.session.add(proveedor); db.session.flush()
    ...
    def editar(self, proveedor_id: int, **datos) -> Proveedor:
        ...
        try:
            anteriores = {"nombre": proveedor.nombre, "identificacion_fiscal": proveedor.identificacion_fiscal}
            self._asignar(proveedor, datos)
    ...
    def eliminar_logicamente(self, proveedor_id: int) -> Proveedor:
        ...
        tiene_productos = db.session.scalar(db.select(db.exists().where(
        ...
        if tiene_historial or tiene_productos:
            raise ErrorProveedor("El proveedor tiene productos o compras asociadas; solo puede desactivarse")
        proveedor.soft_delete(); proveedor.activo = False
    ...
    def _asignar(self, proveedor, datos):
        ...
        try:
            ...
            dias_entrega = int(datos.get("dias_entrega", proveedor.dias_entrega if proveedor.dias_entrega is not None else 7))
        ...
        for campo in ("email", "telefono", "direccion", "ciudad", "sitio_web",
            ...
            if campo in datos:
                setattr(proveedor, campo, (datos[campo] or "").strip() or None)
    ...
    def _auditar(self, proveedor, accion, anteriores=None):
        registrar_auditoria(
            accion=f"proveedor.{accion}", modulo="proveedores", usuario_id=self.usuario.id,
            empresa_id=self.usuario.empresa_id, entidad_tipo="Proveedor", entidad_id=proveedor.id,
