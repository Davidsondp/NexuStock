...
class ServicioExportaciones:
    ...
    def exportar(self, reporte: str, formato: str, *, periodo: Periodo,
                 bodega_id=None) -> tuple[BytesIO, str, str]:
        ...
        contenido = self._csv(encabezados, filas) if formato == "csv" else self._xlsx(encabezados, filas, reporte)
        registrar_auditoria(
            accion="reporte.exportado", modulo="reportes", usuario_id=self.usuario.id,
            empresa_id=self.usuario.empresa_id, entidad_tipo="Reporte",
            datos_nuevos={"reporte": reporte, "formato": formato, "filas": len(filas),
                          "bodega_id": bodega_id, "desde": periodo.desde.date().isoformat(),
                          "hasta": periodo.hasta.date().isoformat()},
        )
        db.session.commit()
        fecha = periodo.hasta.date().isoformat()
        nombre = f"nexustock_{reporte}_{fecha}.{formato}"
        mime = "text/csv; charset=utf-8" if formato == "csv" else \
    ...
    def _datos(self, reporte, periodo, bodega_id):
        ...
        if reporte == "productos":
            return (["Código", "Nombre", "Categoría", "Marca", "Unidad", "Costo referencia", "Precio venta"],
                    [[p.codigo, p.nombre, p.categoria, p.marca, p.unidad_medida,
                      p.costo_referencia, p.precio_venta] for p in servicio.productos()])
        if reporte == "stock":
            return (["Código", "Producto", "Bodega", "Cantidad", "Reservada", "Disponible",
                     "Costo promedio", "Valor inventario"],
                    [[p.codigo, p.nombre, b.nombre, i.cantidad, i.cantidad_reservada,
                      i.cantidad_disponible, i.costo_promedio, i.cantidad * i.costo_promedio]
                     for i, p, b in servicio.stock(bodega_id=bodega_id)])
        if reporte == "movimientos":
            return (["Fecha UTC", "Tipo", "Producto ID", "Bodega ID", "Cantidad", "Stock anterior",
                     "Stock nuevo", "Costo unitario", "Precio unitario", "Motivo"],
                    [[m.fecha.isoformat(), m.tipo, m.producto_id, m.bodega_id, m.cantidad,
                      m.stock_anterior, m.stock_nuevo, m.costo_unitario, m.precio_unitario, m.motivo]
        ...
        return (["Indicador", "Valor"], [
            ["Desde", datos["periodo"]["desde"]], ["Hasta", datos["periodo"]["hasta"]],
            ["Ventas confirmadas", datos["ventas_confirmadas"]], ["Ingresos", datos["ingresos"]],
            ["Costo de ventas", datos["costo_ventas"]], ["Margen bruto", datos["margen_bruto"]],
    ...
    def _xlsx(encabezados, filas, titulo):
        ...
        for indice_fila, fila in enumerate(todas, 1):
            ...
            for indice_columna, valor in enumerate(fila, 1):
                ...
                celdas.append(f'<c r="{referencia}" t="inlineStr"{estilo}><is><t xml:space="preserve">{escape(seguro)}</t></is></c>')
        ...
        with ZipFile(salida, "w", ZIP_DEFLATED) as archivo:
            for nombre, contenido in archivos.items(): archivo.writestr(nombre, contenido)
        salida.seek(0); return salida

    @staticmethod
    def _columna(numero):
        resultado = ""
        while numero:
            numero, resto = divmod(numero - 1, 26); resultado = chr(65 + resto) + resultado
        return resultado
