...
from app.models import (Bodega, Inventario, Movimiento, Producto, Sucursal,
                        Transferencia, Usuario, UsuarioSucursal, Empresa, db)
...
from app.services.transferencias import (ErrorTransferencia, EstadoTransferenciaInvalido,
...
def _preparar(app, client):
    ...
    with app.app_context():
        ...
        db.session.add(sucursal_destino); db.session.flush()
        destino = Bodega(empresa_id=usuario.empresa_id, sucursal_id=sucursal_destino.id,
                         codigo="DEST", nombre="Bodega destino")
        producto = Producto(empresa_id=usuario.empresa_id, codigo="TR-1", nombre="Transferible",
                            costo_referencia=100, precio_venta=200)
        db.session.add_all([destino, producto]); db.session.flush()
        db.session.add(UsuarioSucursal(empresa_id=usuario.empresa_id, usuario_id=usuario.id,
...
def _crear(ids, cantidad=4):
    return ServicioTransferencias(db.session.get(Usuario, ids[0])).crear(
        numero="T-001", bodega_origen_id=ids[1], bodega_destino_id=ids[2],
...
def test_solicitud_no_modifica_stock(app, client):
    ...
    with app.app_context():
        ...
        assert db.session.scalar(db.select(Inventario.cantidad).where(
            Inventario.bodega_id == ids[1])) == 10
...
def test_despacho_parcial_y_recepcion_con_diferencia(app, client):
    ...
    with app.app_context():
        servicio = ServicioTransferencias(db.session.get(Usuario, ids[0]))
        transferencia = _crear(ids, 8); servicio.solicitar(transferencia.id)
        ...
        assert db.session.scalar(db.select(Inventario.cantidad).where(Inventario.bodega_id == ids[2])) == 5
...
def test_fallo_en_despacho_revierte_todos_los_items(app, client):
    ...
    with app.app_context():
        ...
        segundo = Producto(empresa_id=usuario.empresa_id, codigo="TR-2", nombre="Sin stock",
        ...
        transferencia = servicio.crear(
            numero="T-002", bodega_origen_id=ids[1], bodega_destino_id=ids[2],
            items=[{"producto_id": ids[3], "cantidad": 2},
        ...
        assert db.session.get(Transferencia, transferencia.id).estado == "solicitada"
        assert db.session.scalar(db.select(Inventario.cantidad).where(
            Inventario.bodega_id == ids[1], Inventario.producto_id == ids[3])) == 10
...
def test_no_cancela_despues_del_despacho(app, client):
    ...
    with app.app_context():
        servicio = ServicioTransferencias(db.session.get(Usuario, ids[0]))
        transferencia = _crear(ids); servicio.solicitar(transferencia.id); servicio.despachar(transferencia.id)
...
def test_bodega_de_otra_empresa_es_rechazada(app, client):
    ...
    with app.app_context():
        ...
        bodega = Bodega(empresa_id=otra.id, sucursal_id=sucursal.id,
                        codigo="AJENA", nombre="Bodega ajena")
        db.session.add(bodega); db.session.commit(); bodega_id = bodega.id
        with pytest.raises(PermissionError):
            ServicioTransferencias(db.session.get(Usuario, ids[0])).crear(
                numero="T-AJENA", bodega_origen_id=ids[1], bodega_destino_id=bodega_id,
...
def test_bodega_no_asignada_de_la_misma_empresa_es_rechazada(app, client):
    ...
    with app.app_context():
        ...
        db.session.add(sucursal); db.session.flush()
        bodega = Bodega(empresa_id=usuario.empresa_id, sucursal_id=sucursal.id,
                        codigo="NO-ASIG", nombre="No asignada")
        db.session.add(bodega); db.session.commit(); bodega_id = bodega.id
        with pytest.raises(PermissionError):
            ServicioTransferencias(usuario).crear(
                numero="T-NO-ASIG", bodega_origen_id=ids[1], bodega_destino_id=bodega_id,
                items=[{"producto_id": ids[3], "cantidad": 1}],
            )
