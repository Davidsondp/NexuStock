...
def test_api_edita_cliente(client):
    ...
    assert cliente["direccion"] == "Nueva direcci?n 456"
...
def test_api_busca_desactiva_y_reactiva_cliente(client):
    ...
    nombres = {
        cliente["nombre"]
        for cliente
        in respuesta_busqueda.get_json()["clientes"]
    }

    assert nombres == {"Paciente Austral"}

    desactivacion = client.post(
        f"/api/clientes/{cliente_id}/desactivar"
    )
    ...
    ids_activos = {
        cliente["id"]
        for cliente in listado_activos.get_json()["clientes"]
    }
    ...
    reactivacion = client.post(
        f"/api/clientes/{cliente_id}/reactivar"
    )
...
def test_api_rechaza_cliente_invalido_y_rut_duplicado(
    ...
    assert (
        respuesta_sin_nombre.get_json()["codigo"]
        == "cliente_invalido"
    )
...
def test_api_elimina_cliente_sin_historial(client):
    ...
    ids = {
        cliente["id"]
        for cliente in listado.get_json()["clientes"]
    }
...
def test_api_clientes_aisla_empresas(client):
    ...
    desactivacion_cruzada = client.post(
        f"/api/clientes/{cliente_id}/desactivar"
    )

    assert desactivacion_cruzada.status_code == 403

    eliminacion_cruzada = client.delete(
        f"/api/clientes/{cliente_id}"
    )
    ...
    assert (
        cliente_b["identificacion_fiscal"]
        == "77.777.777-7"
    )
