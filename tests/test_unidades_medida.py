...
def test_api_edita_presentacion_personalizada(
    ...
    assert creacion.status_code == 201
    presentacion_id = (
        creacion.get_json()["id"]
    )

    respuesta = client.patch(
        f"/api/productos/{producto['id']}"
        f"/presentaciones/{presentacion_id}",
        json={
            "codigo": "PACK-8",
            "nombre": "Pack de 8",
            "abreviatura": "pack",
...
def test_api_desactiva_presentacion_personalizada(
    ...
    respuesta = client.post(
        f"/api/productos/{producto['id']}"
        f"/presentaciones/"
        f"{presentacion['id']}/desactivar"
    )

    assert respuesta.status_code == 200
    assert (
        respuesta.get_json()["activa"]
        is False
    )
