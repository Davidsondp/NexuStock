...
def _preparar(app, client):
    ...
    with app.app_context():
        ...
        usuario.empresa.suscripcion_actual.plan.funciones = {
            **usuario.empresa.suscripcion_actual.plan.funciones, "analitica": True,
            "reportes.avanzados": True}
        bodega = db.session.scalar(db.select(Bodega).where(Bodega.empresa_id == usuario.empresa_id))
        primero = Producto(empresa_id=usuario.empresa_id, codigo="R-1", nombre="Más vendido",
                           costo_referencia=100, precio_venta=200, stock_maximo=100)
        segundo = Producto(empresa_id=usuario.empresa_id, codigo="R-2", nombre="Sobrestock",
                           costo_referencia=50, precio_venta=80, stock_maximo=5)
        db.session.add_all([primero, segundo]); db.session.commit()
        inventario = ServicioInventario(usuario, ContextoOperacion(usuario.empresa_id, bodega.sucursal, bodega))
        ...
        venta = ServicioVentas(usuario).crear(numero="R-VTA", bodega_id=bodega.id,
            items=[{"producto_id": primero.id, "cantidad": 5, "precio_unitario": 200,
                    "impuesto": 190}])
        ServicioVentas(usuario).reservar(venta.id); ServicioVentas(usuario).confirmar(venta.id)
