...
def test_salida_aplica_fefo(
    ...
    with app.app_context():
        ...
        lotes = {
            lote.numero: lote
            for lote in db.session.scalars(
        ...
        assert (
            lotes["LOTE-PROXIMO"].cantidad
            == Decimal("0.000")
        )
        assert (
            lotes["LOTE-TARDIO"].cantidad
            == Decimal("4.000")
        )
...
def test_salida_rechaza_lotes_validos_insuficientes(
    ...
    with app.app_context():
        lote_vencido = db.session.scalar(
            db.select(Lote).where(
                Lote.numero == "LOTE-VENCIDO"
            )
        )
        lote_vencido.fecha_vencimiento = (
        ...
        lote_valido = db.session.scalar(
            db.select(Lote).where(
                Lote.numero == "LOTE-VALIDO"
            )
        )
        inventario = db.session.scalar(
            db.select(Inventario).where(
...
def _preparar_controlado(
    ...
    with app.app_context():
        ...
        assert (
            producto_verificado.controla_lotes
            is True
        )
        assert (
            producto_verificado.controla_vencimiento
            is True
        )

    # La fixture mantiene un contexto exterior;
    # fuerza que la siguiente petici?n consulte
...
def test_venta_confirmada_aplica_fefo_y_trazabilidad(
    ...
    with app.app_context():
        ...
        assert (
            lotes["VENTA-PROXIMO"].cantidad
            == Decimal("0.000")
        )
        assert (
            lotes["VENTA-TARDIO"].cantidad
            == Decimal("4.000")
        )
        ...
        traza_proxima = trazas_por_lote[
            "VENTA-PROXIMO"
        ]
        traza_tardia = trazas_por_lote[
            "VENTA-TARDIO"
        ]

        assert (
            traza_proxima.cantidad
            == Decimal("-5.000")
        )
        assert (
            traza_proxima.saldo_anterior
            == Decimal("5.000")
        )
        assert (
            traza_proxima.saldo_nuevo
            == Decimal("0.000")
        )

        assert (
            traza_tardia.cantidad
            == Decimal("-1.000")
        )
        assert (
            traza_tardia.saldo_anterior
            == Decimal("5.000")
        )
        assert (
            traza_tardia.saldo_nuevo
            == Decimal("4.000")
        )
...
def test_api_entrada_controlada_recibe_lote(
    ...
    respuesta_productos = client.get(
        "/api/productos"
    )

    assert (
        respuesta_productos.status_code
        == 200
    )
    ...
    producto_api = next(
        ...
        if producto["id"] == ids[1]
    )

    assert producto_api["controla_lotes"] is True, (
        producto_api
    )
    assert (
        producto_api["controla_vencimiento"]
        is True
    ), producto_api
...
def test_api_lista_lotes_con_estado_vencimiento(
    ...
    with app.app_context():
        vencido = db.session.scalar(
            db.select(Lote).where(
                Lote.numero == "API-VENCIDO"
            )
        )
        vencido.fecha_vencimiento = (
            hoy - timedelta(days=2)
        )
        db.session.commit()
    ...
    assert (
        lotes["API-VIGENTE"]["cantidad"]
        == "4.000"
    )
    assert (
        lotes["API-VIGENTE"]["valor"]
        == "400.00"
    )
        ...
        alertas = list(
            db.session.scalars(
                ...
                .where(
                    ...
                    == usuario.empresa_id,
                    AlertaInventario.lote_id.is_not(
                        None
                    ),
    ...
    with app.app_context():
        ...
        cantidad = db.session.scalar(
            db.select(
                ...
                AlertaInventario.lote_id
                == lote.id,
                AlertaInventario.tipo
                == "lote_proximo_vencer",
                AlertaInventario.estado
                == "activa",
            )
...
def test_api_alertas_expone_informacion_del_lote(
    ...
    assert (
        alerta["datos"]["numero_lote"]
        == "API-ALERTA-LOTE"
    )
    assert (
        alerta["datos"]["dias_para_vencer"]
        == 12
    )
    assert (
        alerta["datos"]["cantidad"]
        == "6.000"
    )
    assert (
        alerta["datos"]["fecha_vencimiento"]
        == (
            date.today()
            + timedelta(days=12)
        ).isoformat()
...
def test_comando_automatico_genera_alertas_sin_sesion(
    ...
    primera = runner.invoke(
        args=["generar-alertas"]
    )

    assert primera.exit_code == 0, (
        primera.output
        or repr(primera.exception)
    )
    ...
    assert segunda.exit_code == 0, (
        segunda.output
        or repr(segunda.exception)
    )

    with app.app_context():
        lote = db.session.scalar(
            db.select(Lote).where(
                Lote.numero == "CRON-LOTE-001"
            )
        )
...
def test_comando_automatico_aisla_varias_empresas(
    ...
    assert resultado.exit_code == 0, (
        resultado.output
        or repr(resultado.exception)
    )
    assert (
        "Empresas procesadas: 1"
        in resultado.output
    )
    assert (
        "Empresas omitidas: 1"
