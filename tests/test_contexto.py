...
def test_no_permita_sucursal_de_otra_empresa(app, client):
    ...
    with app.app_context():
        ...
        db.session.add(bodega); db.session.commit()
...
def test_api_bodegas_no_expone_otra_empresa(app, client):
    ...
    with app.app_context():
        ...
        db.session.add(sucursal); db.session.flush()
        db.session.add(Bodega(empresa_id=otra.id, sucursal_id=sucursal.id,
                              codigo="OTRA", nombre="Secreta"))
        db.session.commit(); sucursal_id = sucursal.id
...
def test_empleado_solo_ve_sucursales_asignadas(app, client):
    ...
    with app.app_context():
        ...
        db.session.add(segunda); db.session.flush()
        db.session.add(Bodega(empresa_id=admin.empresa_id, sucursal_id=segunda.id,
                              codigo="DOS", nombre="Bodega dos"))
        empleado = Usuario(empresa_id=admin.empresa_id, nombre="Empleado",
                           email="empleado@nexustock.cl", rol="empleado")
        empleado.set_password("ClaveEmpleado123")
        db.session.add(empleado); db.session.flush()
        principal = db.session.scalar(db.select(Sucursal).where(Sucursal.codigo == "PRINCIPAL"))
        db.session.add(UsuarioSucursal(empresa_id=admin.empresa_id, usuario_id=empleado.id,
                                      sucursal_id=principal.id, es_principal=True))
        db.session.commit()
    client.post("/autenticacion/salir")
    client.post("/autenticacion/ingresar", data={"email": "empleado@nexustock.cl",
...
def test_seleccion_valida_guarda_contexto_y_audita(app, client):
    ...
    respuesta = client.post("/contexto/seleccionar", data={
        "sucursal_id": ids[0], "bodega_id": ids[1]
    })
