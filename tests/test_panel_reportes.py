...
def test_panel_reportes_expone_interfaz_empresarial(
    ...
    contratos = (
        ...
        (
            b'data-api-movimientos="'
            b'/api/reportes/movimientos"'
        ),
        (
            b'data-api-analitica="'
            b'/api/reportes/analitica"'
        ),
        b'data-bodega-id="',
...
def test_paneles_enlazan_centro_reportes():
    ...
    for nombre in plantillas:
        contenido = Path(
            "app/templates/panel"
        ).joinpath(nombre).read_text(
            encoding="utf-8-sig"
        )

        assert (
            "url_for('panel.reportes')"
            in contenido
            or (
                'url_for(\n'
                "                        "
