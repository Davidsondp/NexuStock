...
def test_csv_stock_incluye_bom_y_separador_chileno(app, client):
    ids = _preparar(app, client); _habilitar(app, ids)
...
def test_neutraliza_inyeccion_de_formulas_csv_y_excel(app, client):
    ids = _preparar(app, client); _habilitar(app, ids)
    with app.app_context():
        producto = db.session.get(Producto, ids[2]); producto.nombre = "=HIPERVINCULO(\"sitio\")"
        db.session.commit()
...
def test_exportacion_registra_auditoria(app, client):
    ...
    with app.app_context():
        auditoria = db.session.scalar(db.select(Auditoria).where(
            Auditoria.accion == "reporte.exportado"))
