...
REGISTRO = {
    ...
    "identificacion_fiscal": "76.123.456-7",
    "nombre": "Davidson",
    "apellido": "Pierre",
...
def test_registro_inicial_es_atomico_y_completo(app, client):
    ...
    with app.app_context():
        ...
        assert usuario.rol == "admin_empresa"
...
def test_login_bloquea_tras_cinco_fallos(app, client):
    ...
    client.post("/autenticacion/salir")
    for _ in range(5):
        client.post("/autenticacion/ingresar", data={"email": REGISTRO["email"], "password": "incorrecta"})
    with app.app_context():
        usuario = db.session.scalar(db.select(Usuario))
...
def test_login_correcto_reinicia_intentos(app, client):
    ...
    client.post("/autenticacion/salir")
    client.post("/autenticacion/ingresar", data={"email": REGISTRO["email"], "password": "incorrecta"})
    respuesta = client.post("/autenticacion/ingresar", data={"email": REGISTRO["email"],
...
def test_token_es_de_un_solo_uso(app, client):
    ...
    with correo.record_messages() as enviados:
        ...
        enlace = [linea for linea in enviados[0].body.splitlines() if "/restablecer-password/" in linea][0]
        ruta = enlace.split("localhost", 1)[1]
    respuesta = client.post(ruta, data={"password": "NuevaClave123",
...
def test_cambio_password_invalida_otra_sesion(app, client):
    ...
    client.post("/autenticacion/registro", data=REGISTRO)
    otro_navegador.post("/autenticacion/ingresar", data={
        "email": REGISTRO["email"], "password": REGISTRO["password"]
    })
    client.post("/autenticacion/salir")
    with correo.record_messages() as enviados:
        client.post("/autenticacion/olvide-password", data={"email": REGISTRO["email"]})
        enlace = [linea for linea in enviados[0].body.splitlines() if "/restablecer-password/" in linea][0]
    client.post(enlace.split("localhost", 1)[1], data={
        "password": "NuevaClave123", "confirmar_password": "NuevaClave123"
    })
