...
def test_recepcion_por_presentacion_convierte_a_base(
    ...
    with app.app_context():
        ...
        assert (
            item_recepcion.cantidad
            == Decimal("12.000")
        )
        assert (
            item_recepcion.cantidad_presentacion
            == Decimal("1.000")
        )
        assert (
            item_recepcion.factor_conversion
            == Decimal("12.000")
        )
        assert (
            item_recepcion.costo_presentacion
            == Decimal("12000.0000")
        )
        assert (
            item_recepcion.costo_unitario
            == Decimal("1000.0000")
        )
        assert (
            item_orden.cantidad_recibida
            == Decimal("12.000")
        )
        assert (
            orden.estado
...
def test_api_recepcion_expone_conversion_comercial(
    ...
    assert (
        client.post(
            f"/api/compras/{orden_id}/confirmar"
        ).status_code
        == 200
    )
    assert (
        client.post(
            f"/api/compras/{orden_id}/enviar"
        ).status_code
        == 200
    )
