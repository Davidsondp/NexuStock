...
def test_id_solicitud_invalido_no_se_refleja(app, client):
    respuesta = client.get("/api/ruta-inexistente", headers={"X-Request-ID": "<script>alert(1)</script>"})
...
def test_api_exige_json_en_operaciones_con_cuerpo(app, client):
    ...
    respuesta = client.post("/api/productos", data="nombre=Producto",
...
def test_errores_404_y_405_no_filtran_detalles(app, client):
    ...
    assert no_encontrado.status_code == 404 and no_encontrado.get_json()["codigo"] == "no_encontrado"
...
def test_limite_login_es_persistente_y_no_guarda_ip(app, client):
    for _ in range(10):
        respuesta = client.post("/autenticacion/ingresar", data={
            "email": "nadie@nexustock.cl", "password": "incorrecta"})
        assert respuesta.status_code == 200
    bloqueada = client.post("/autenticacion/ingresar", data={
        "email": "nadie@nexustock.cl", "password": "incorrecta"})
...
def test_cookie_de_sesion_tiene_atributos_defensivos(app, client):
    ...
    cookie = next(c for c in respuesta.headers.getlist("Set-Cookie") if c.startswith("nexustock_sesion="))
...
def test_payload_demasiado_grande_es_rechazado(app, client):
    ...
    respuesta = client.post("/api/productos", data=b'{"nombre":"' + b"x" * 100 + b'"}',
                            content_type="application/json")
    assert respuesta.status_code == 413
    assert respuesta.get_json()["codigo"] == "contenido_demasiado_grande"


def test_webhook_sigue_exento_de_csrf_pero_exige_firma(app, client):
    app.config["WTF_CSRF_ENABLED"] = True
    app.config["WEBHOOK_PAGOS_SECRET"] = "secreto-webhook-seguridad-1234567890"
    respuesta = client.post("/webhooks/pagos/webpay", data=b"{}", content_type="application/json")
    assert respuesta.status_code == 401
    assert respuesta.get_json()["codigo"] == "firma_webhook_invalida"