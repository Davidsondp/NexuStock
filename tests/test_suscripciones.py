...
def test_solo_una_solicitud_pendiente(app, client):
    ...
    with app.app_context():
        ...
        with pytest.raises(ErrorSuscripcion):
            servicio.solicitar_cambio(plan_codigo="profesional-pago", ciclo="anual")
...
def test_api_webhook_y_auditoria(app, client):
    ...
    solicitud = client.post(
        ...
        json={"plan_codigo": "profesional-pago", "ciclo": "mensual"},
    ).get_json()
    client.post(
        f"/api/suscripciones/solicitudes/{solicitud['id']}/pagos",
        json={"proveedor": "webpay", "referencia_externa": "WP-001"},
    )
...
def test_resumen_expone_planes_comerciales_activos(
    ...
    plan = next(plan for plan in planes if plan["codigo"] == "basico-panel")
...
def test_resumen_ordena_planes_comerciales(
    ...
    assert codigos.index("primero") < codigos.index("ultimo")
...
def test_planes_disponibles_exponen_matriz_completa(
    ...
    profesional = next(plan for plan in planes if plan["codigo"] == "profesional-pago")
