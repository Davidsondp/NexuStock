...
def test_panel_alertas_expone_interfaz_empresarial(
    ...
    for texto_unicode in textos_unicode:
        assert (
            texto_unicode.encode("utf-8")
            in respuesta.data
        )
    assert (
        b'data-api-alertas="/api/alertas"'
        in respuesta.data
    )
    assert (
        b'class="principal"'
    ...
    assert (
        b'class="superposicion"'
        in respuesta.data
    )
    assert b'id="fondo-menu"' not in respuesta.data
    assert (
        b'<section class="contenido">'
    ...
    assert (
        b'id="filtro-estado-alertas"'
        in respuesta.data
    )
    assert (
        b'id="filtro-tipo-alertas"'
        in respuesta.data
    )
    assert (
        b'id="resumen-alertas-activas"'
        in respuesta.data
    )
    assert (
        b'id="resumen-alertas-criticas"'
        in respuesta.data
    )
    assert (
        b'id="resumen-alertas-altas"'
        in respuesta.data
    )
    assert (
        b'js/alertas.js'
...
def test_empleado_solo_consulta_alertas(
    ...
    assert respuesta.status_code == 200
    assert (
        b'data-permiso-gestionar="false"'
        in respuesta.data
    )
