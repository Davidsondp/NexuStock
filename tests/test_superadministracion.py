...
def test_superadmin_edita_plan_desde_api_y_panel(app, client):
    ...
    with app.app_context():
        plan_id = db.session.scalar(db.select(PlanSaaS.id))

    respuesta = client.patch(
        f"/api/superadmin/planes/{plan_id}",
        json={
            "nombre": "Prueba Comercial",
    ...
    with app.app_context():
        ...
        assert plan.nombre == "Prueba Comercial"
        assert plan.precio_mensual == 14990
