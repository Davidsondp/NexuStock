...
def test_javascript_autenticacion_controla_password():
    contenido = Path(
        "app/static/js/autenticacion.js"
    ).read_text(encoding="utf-8-sig")
...
def test_formulario_rubros_conserva_unicode():
    contenido = Path(
        "app/blueprints/autenticacion/forms.py"
    ).read_text(encoding="utf-8-sig")
    ...
    assert "Almac?n" not in contenido
    assert "Botiller?a" not in contenido
    assert "Ferreter?a" not in contenido
