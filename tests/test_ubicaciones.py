...
def test_api_lista_ubicaciones_inactivas_y_detalles(
    ...
    encontrada = next(
        item
        for item in bodegas
        if item["id"] == bodega_id
    )

    assert encontrada["descripcion"] == (
        "Despacho mayorista"
    )
    assert encontrada["activa"] is False
...
def test_api_reactiva_sucursal_y_bodega(
    ...
    bodega = next(
        item
        for item in respuesta.get_json()["bodegas"]
        if item["id"] == bodega_id
    )
    ...
    with app.app_context():
        ...
        bodega = db.session.scalar(
            db.select(Bodega).where(
                Bodega.codigo == "REACT-2"
            )
        )

        servicio.desactivar_bodega(
            bodega.id
        )
        segundo_id = bodega.id

    respuesta = client.post(
        f"/api/bodegas/{segundo_id}"
        "/reactivar"
    )
...
def test_api_desasigna_usuario_de_sucursal(
    ...
    respuesta = client.delete(
        f"/api/sucursales/{segunda_id}"
        f"/usuarios/{empleado_id}"
    )
